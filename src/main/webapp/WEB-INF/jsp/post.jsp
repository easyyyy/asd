	...
	<div id="page-wrapper">
		...
		<!-- 搜索部分 -->
			<div class="panel-body">
				<form class="form-inline" method="get" action="post/find">
					<div class="form-group">
						<label for="depid">部门</label> 
						<select class="form-control" id="depid" name="depID">

							<c:forEach items="${departments}" var="d" varStatus="st">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="name" value="" name="postName">
					</div>
					...
					<table class="table table-bordered table-striped">
						...
						<c:forEach items="${posts}" var="c" varStatus="st">
							<tr>
								...
								<td>
									<a class="btn btn-primary btn-xs" data-toggle="modal" data-target="#crmpostEditDialog" onclick="editcrmpost(${c.postID})">修改</a>
									<button class="btn btn-danger btn-xs" onclick="deletecrmpost(${c.postID})">删除</button>
								</td>
...
//清空新建职务窗口中的数据
	...
	//     });
		...
		$.ajax({
			type:"POST",
			url:"post/add",
			data:$("#new_crmpost_form").serialize(),
			success:function () {
				alert("新建成功！");
				window.location.reload();
	...
	function updatecrmpost() {
		...
		$.ajax({
			type:"POST",
			url:"post/updatePost",
			data:$("#edit_crmpost_form").serialize(),
			dataType:"text",
			success:function() {
				alert("修改成功!");
				window.location.reload();
